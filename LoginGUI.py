...
def password_sign_in(username, username_frame):
    ...
    view_pword_toggle = Button(password_frame, image=open_eye, width=30, height=22, command=lambda: switch_eye_image(view_pword_toggle, password_entry, password_input, password_input, password_frame))
    ...
    Button(password_frame, text="Sign In", font=("Calibri Bold", 13), bg="#258cdb", command=lambda: confirm_password(password_input.get(), pword_warning_text)).place(x=330,y=310)
...
def confirm_password(password_input, warning_text):
